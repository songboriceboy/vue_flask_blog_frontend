...
<section>
  <div class="container">
    <nav class="navbar navbar-expand-lg navbar-light bg-light" style="margin-bottom: 20px;">
      ...
      <a href="http://www.xxx.com" class="g-text-underline--none--hover">xxx</a>
      </div>
  ...
  mounted () {
    ...
    $(function() {
      ...
      setInterval(function() {
        if (window.localStorage.getItem('blog-token')) {
          // 如果用户已登录，才开始请求 API
          const payload = JSON.parse(atob(window.localStorage.getItem('blog-token').split('.')[1]))
